...
///
/// Layering: every typed field in `Radio` / `Wifi` is applied if `Some`,
/// otherwise falls through to a sensible band/security-derived default.
/// `extra` lines are appended verbatim at the very end so operators can
/// pass arbitrary hostapd options without waiting for us to surface them
/// as typed fields.
fn build_hostapd_conf(radio: &Radio, w: &Wifi) -> String {
    ...
    // ── phy-level: hw_mode, channel, 802.11n/ac/ax, HT/VHT/HE ──
    let is_5g = radio.band.as_str() != "2g"; // default 5g for anything non-2g
    if is_5g {
    ...
    // Defaults: n always on; ac on 5g only; ax on both.
    let n_default = true;
    ...
    // HT capability list: operator override wins, otherwise band default.
    ...
    // ── BSS-level: security + MFP + optional knobs ──
    // Default-derive wpa_key_mgmt + MFP from `security`; typed
    // overrides win.
