...
pub fn write_all(cfg: &Config) -> io::Result<()> {
    ...
    if let Ok(rd) = std::fs::read_dir(dir) {
        for ent in rd.flatten() {
            let p = ent.path();
            if p.extension().and_then(|s| s.to_str()) == Some("conf") && !desired.contains(&p) {
...
fn build_hostapd_conf(radio: &Radio, wifis: &[&Wifi]) -> String {
    assert!(
        !wifis.is_empty(),
        "build_hostapd_conf called with no wifi entries"
    );

    let mut s = String::with_capacity(2048);
    writeln!(
        s,
        "# Auto-generated by oxwrtd wifi::generate_all; do not edit."
    )
    .unwrap();
    writeln!(
        s,
        "# Source: [[radios]] phy={} + {} [[wifi]] entries",
        radio.phy,
        wifis.len()
    )
    .unwrap();
...
fn write_phy_block(s: &mut String, radio: &Radio) {
    ...
    writeln!(
        s,
        "ieee80211d={}",
        bool_to_01(radio.ieee80211d.unwrap_or(true))
    )
    .unwrap();
    writeln!(
        s,
        "ieee80211h={}",
        bool_to_01(radio.ieee80211h.unwrap_or(true))
    )
    .unwrap();
    ...
    writeln!(
        s,
        "ieee80211n={}",
        bool_to_01(radio.ieee80211n.unwrap_or(n_default))
    )
    .unwrap();
    ...
    let ht_capab = radio.ht_capab.as_deref().unwrap_or(if is_5g {
        ht_capab_default_5g
    } else {
        ht_capab_default_2g
    });
...
fn write_bss_security_block(s: &mut String, w: &Wifi) {
    let (default_wpa, default_key_mgmt, default_mfp, default_sae_require): (u8, &str, u8, bool) =
        match w.security {
    ...
    writeln!(
        s,
        "ap_isolate={}",
        bool_to_01(w.ap_isolate.unwrap_or(false))
    )
    .unwrap();
