...
pub fn run(args: Vec<String>) -> Result<(), String> {
    ...
    if let Some(path) = out_path {
        ...
        eprintln!("Next: copy both to /etc/oxwrt/ on the router, then\n  `oxctl <remote> reload`.");
    } else {
...
fn write_file_mode(path: &std::path::Path, text: &str, mode: u32) -> Result<(), String> {
...
fn render(i: &Inputs) -> String {
    ...
    writeln!(
        s,
        "# config/oxwrt.toml in the upstream repo for every knob + docs.\n"
    )
    .unwrap();
...
fn parse_cidr_v4(s: &str) -> Result<(String, u8), String> {
    ...
    let p: u8 = pref
        .parse()
        .map_err(|e| format!("bad prefix {pref:?}: {e}"))?;
    if p > 32 {
